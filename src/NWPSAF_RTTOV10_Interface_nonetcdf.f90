...
! Ticket  Date     Comment
! ------- -------- -------
! 25      20/02/12 Original version, based on RTTOV10_interface without 
...
!write(*,*) 'AVANT APPEL RTTOV valid channels', Valid_Channels(:,I)
 ...
 !    Soft_Limits % Minimum(Prof_FirstQ:Prof_LastQ) = &
  !        LOG(RTTOV10_Coefs(1) % coef % lim_prfl_gmin(:,wv_pos)/ &
   !       H2O_MassMixToPPMV)-0.5
     Soft_Limits % Minimum(Prof_FirstQ:Prof_LastQ) = &
          RTTOV10_Coefs(1) % coef % lim_prfl_gmin(:,wv_pos)/ &
          H2O_MassMixToPPMV
   ...
   !  Soft_Limits % Minimum(Prof_Q2) = &
    !      LOG(RTTOV10_Coefs(1) % coef % lim_prfl_gmin(Num_RTLevels,wv_pos)/ &
     !     H2O_MassMixToPPMV)-0.5
    Soft_Limits % Minimum(Prof_Q2) = &
          RTTOV10_Coefs(1) % coef % lim_prfl_gmin(Num_RTLevels,wv_pos)/ &
          H2O_MassMixToPPMV
  ...
  !   Soft_Limits % Maximum(Prof_FirstQ:Prof_LastQ) = &
   !       LOG(RTTOV10_Coefs(1) % coef % lim_prfl_gmax(:,wv_pos)/ &
    !      H2O_MassMixToPPMV)+0.5
     Soft_Limits % Maximum(Prof_FirstQ:Prof_LastQ) = &
          RTTOV10_Coefs(1) % coef % lim_prfl_gmax(:,wv_pos)/ &
          H2O_MassMixToPPMV
 ...
 !    Soft_Limits % Maximum(Prof_Q2) = &
  !        LOG(RTTOV10_Coefs(1) % coef % lim_prfl_gmax(Num_RTLevels,wv_pos)/ &
   !       H2O_MassMixToPPMV)+0.5
     Soft_Limits % Maximum(Prof_Q2) = &
          RTTOV10_Coefs(1) % coef % lim_prfl_gmax(Num_RTLevels,wv_pos)/ &
          H2O_MassMixToPPMV
...
!Do i=1,54
!	write(*,*) 'RTTOV DIR', profiles(1)%p(i),profiles(1)%t(i),profiles(1)%q(i)
!ENDDO
...
!====NORMALEMENT MODIFICATION ICI SI ON NEST PAS EN LN Q MAIS JAI LIMPRESSION DUNE ERREUR IL MANQUE LE MULTIPLIER PAR Q A VERIFIER
              DO j=1,Num_WetLevels
                 ! Kmat= dTB/dln(qtotal)=qtotal dTB/dqtotal
!POUR MOI ON NE CALCULE ICI QUE dTB/dqtotal
                 DO i=First_Chan_Pos,Last_Chan_Pos
