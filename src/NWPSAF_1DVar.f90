...
CHARACTER(LEN=80) :: Message(39)      ! Message for Gen_MessageReport
...
IF (LastOb > Obs% header% NumObsLocal) THEN
   ErrorMessage(1) = 'Resetting LastOb to be less than the number'
   ErrorMessage(2) = 'of obs in the Obsfile'
...
IF (LastOb < FirstOb) THEN
   ErrorMessage(1) = 'LastOb is less than FirstOb'
   ErrorMessage(2) = ''
...
!----------------------------------------
!4. Additional output routines (optional)
!----------------------------------------
...
IF ( GeneralMode >= DiagnosticMode ) THEN
   ...
   Message(message_number) = '**** Finished 1DVar ****'
   WRITE( UNIT=Message(1), FMT='(A)' ) &
        Message(message_number)
   CALL Gen_MessageReport( RoutineName,Message(1:message_number) )
END IF
