...
DO IChan=1,UsedChans % NumChans
  ...
  zteff = RT_Params % tc2(Ichan) * BTObserved(Channel) + &
       RT_Params % tc1(IChan)

  ObservedRadiance(IChan) = RT_Params % bcon1(IChan)&
        / (exp(RT_Params % bcon2(IChan)/zteff) - 1.0)

  zteff2 = LOG(1.0 + (RT_Params % bcon1(IChan) &
               / RT_Params % TotalRadiances(IChan)))

  sigma = RT_Params % TotalRadiances(IChan)&
        * (RT_Params % bcon1(IChan) &
        + RT_Params % TotalRadiances(IChan))&
        * RT_Params % tc2(IChan) &
        /(RT_Params % bcon1(IChan)&
          * RT_Params % bcon2(IChan))&
        * zteff2 * zteff2
...
DO IPress_Lev = TropLevel,Num_RTLevels-1
  ...
  DO IChan = 1, UsedChans % NumChans
    ...
    ELSE IF (UsedChans % Channels(IChan) == &
         DetectCloudChans % Channels(JChan)) THEN
      MixedRadiance(IChan,IPress_Lev)= RT_Params % TotalRadiances(IChan)&
                      * (1.0 - Cld_Frac_Vec(IPress_Lev))&
                      + RT_Params % CloudyRadiances(IChan,IPress_Lev)&
                      * Cld_Frac_Vec(IPress_Lev)

      Cost_Func_Inc_Root = ObservedRadiance(IChan)&
                       - MixedRadiance(IChan,IPress_Lev)
      Cost_Func_Inc = Cost_Func_Inc_Root * Cost_Func_Inc_Root&
                  / sigma2(IChan)
...
DO IChan = 1,UsedChans % NumChans
  ...
  zteff = RT_Params % bcon2(IChan)&
        /ALOG( 1.0 + RT_Params % bcon1(IChan) &
        /RT_Params % TotalRadiances(IChan))

  RT_Params % TotalBTs(IChan) = (zteff &
             - RT_Params % tc1(IChan)) &
             / RT_Params % tc2(IChan)
END DO
