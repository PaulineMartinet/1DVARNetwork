    SUBROUTINE NWPSAF_SetUpRetrievals(RTModel)
...
USE NWPSAFMod_RTModel, ONLY: &
     ...
     RTModel_Name, RTTOV, RTTOV8, RTTOV9, RTTOV10, RTTOV11
...
! Local variables:
CHARACTER(LEN=80)  :: ErrorMessage(3)=' '  ! Message for Gen_ErrorReport
...
INTEGER :: Integrated_Water_Vapor(2)  = (/ 0, 0 /)  !PM change         
...
NAMELIST / Retrieval / &
     ...
     Skin_Temperature,    &
     Cloud_Liquid_Water,  &  
     Integrated_Water_Vapor,  &       !PM change      
     Cloud_Top_Pressure,  &
     Cloud_Fraction,      &
     Surface_UWind,       &
     Surface_VWind
...
IF (Cloud_Liquid_Water(1) /= 0) THEN
   ...
   IF (Lqtotal == 0) THEN
      Num_ProfElementsUsed = Num_ProfElementsUsed + 1
      Retrieved_Elements_Tmp(Num_ProfElementsUsed) = Prof_CLW 
...
! Cloud Liquid Water
IF (Integrated_Water_Vapor(1) /= 0) THEN
   ! Cloud_Liquid_Water(1) is non-zero if this element is to be retrieved.
   ! Cloud_Liquid_Water(2) is the number of the B-matrix element 
   ! corresponding to Cloud_Liquid_Water.
   IWVRetrieval = .TRUE.
  ! IF (Lqtotal == 0) THEN
      Num_ProfElementsUsed = Num_ProfElementsUsed + 1
      Retrieved_Elements_Tmp(Num_ProfElementsUsed) = Prof_IWV
      B_ElementsUsed_Tmp(Num_ProfElementsUsed) =  Integrated_Water_Vapor(2)
      Ret_IWV = Num_ProfElementsUsed
 !  END IF
 END IF
...
! Ensure that if cloudy retrievals are required we have the right RT
! model to do it.

IF ( CloudyRetrieval .AND. ((RTModel /= RTTOV) .AND. (RTModel /= RTTOV8) &
    .AND. (RTModel /= RTTOV9) .AND. (RTModel /= RTTOV10) .AND. &
    (RTModel /= RTTOV11)) ) THEN
  Errormessage(1) = ' Cloud retrievals are not set up with '// &
       RTModel_Name(RTModel)
  ErrStatRep = StatusFatal
  CALL Gen_ErrorReport( RoutineName,  &
       ErrorMessage, &
       ErrStatRep    )
