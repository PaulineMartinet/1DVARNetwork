...
CHARACTER(LEN=80) :: Message(3)       ! message for Gen_MessageReport
...
!------------------------------------------------------------------------------
  ...
  Read_matrices: DO  ! read data until end of file reached 
    ...
    READ (file_unit,FMT='(A20)', IOSTAT=readstatus ) &
         Instrument_Name
    IF (readstatus < 0) THEN
       End_of_File = .TRUE.
    ELSE IF ( readstatus > 0 ) THEN
     WRITE( UNIT=ErrorMessage(1),FMT='(A)' ) &
            'Error Reading Instrument Name'
    ...
    IF (.NOT.(End_Of_File)) THEN
       ...
       ELSE IF ( readstatus /= 0 ) THEN
          WRITE( UNIT=ErrorMessage(1),FMT='(A)' ) &
               'Error Reading Num_Channels etc.'
          ErrStatRep = StatusFatal
          EXIT Read_matrices
       END IF
    END IF
    ...
    IF ( Rtype == R_Full_Matrix .AND. numRchannels /= numRelements ) THEN
       Errormessage(1) = 'Full Matrix is not Square'
       ErrStatRep = StatusFatal
    ...
    IF (numRchannels * numRelements > 10000000) THEN
       Errormessage(1) = 'R matrix is too big'
       WRITE( UNIT=ErrorMessage(2),FMT='(A29,2I7)' ) &
    ...
    IF ( .NOT.(End_of_File) ) THEN
       ...
       IF ( readstatus /= 0 ) THEN
          WRITE( UNIT=ErrorMessage(1),FMT='(A)' ) &
               'R matrix I/O error (second read)'
          ErrStatRep = StatusFatal
          EXIT Read_matrices
       ELSE
          ...
          Read_data : DO channel_i = 1, numRelements
             ...
             IF ( readstatus /= 0 ) THEN
                WRITE( UNIT=ErrorMessage(1),FMT='(A)' ) &
                     'R matrix I/O error (third read) at element'
                WRITE( UNIT=ErrorMessage(2),FMT='(A1,I4,A1,I4,A1)' ) &
                     '(',channel_i,',', channel_j,')'
                ErrStatRep = StatusFatal
                EXIT Read_matrices
             ELSE IF ( Rtype == R_Eigenvectors ) THEN
                ...
                IF ( readstatus /= 0 ) THEN
                   WRITE( UNIT=ErrorMessage(1),FMT='(A)' ) &
                        'R matrix I/O error (fourth read)'
                   ErrStatRep = StatusFatal
                END IF
             END IF
          END DO Read_data
       ...
       Check_satellite: DO R_index = 1, RT_Params % Num_SatIDs
          ...
          IF ( Trim(Instrument_Name) == &
             ...
             IF (numRchannels /= RT_Params % SatID(R_index) % Num_Channels) THEN
                IF ( GeneralMode >= DebugMode ) THEN
                   WRITE( UNIT=Message(1),FMT='(A)' ) &
                        'Incorrect number of elements for R-matrix in data file:'
                   WRITE( UNIT=Message(2),FMT='(A,I3,A,I3)' ) &
                        'Elements=',numRchannels,', Required=', &
                        RT_Params % SatID(R_index) % Num_Channels
                   CALL Gen_MessageReport( RoutineName,Message )
                   Message(:) = ' '
                END IF
                EXIT Check_satellite   ! Don't use this satellite
             ELSEIF ( numRchannels < total_channels ) THEN
                WRITE( UNIT=Message(1),FMT='(A)' ) &
                     'Encountered R-matrix with too few elements:'
                WRITE( UNIT=Message(2),FMT='(A,I3,A,I3)' ) &
             ...
             ELSEIF ( ANY(channel_number(:) /= &
                ...
                WRITE( UNIT=Message(1),FMT='(A)' ) &
                     'Absolute channel numbers in R-matrix'
             ...
             IF ( .NOT. GotMatrix(R_index) ) THEN
                ...
                CASE (R_Band_Diagonal)  
                   ...
                   ALLOCATE(R_Reference(R_index) % & 
                        Matrix(nchan,0:numRelements-1))
    ...
    ELSE
       IF ( MatricesFound < RT_Params % Num_SatIDs ) THEN 
          ...
          Errormessage(1) = 'EOF before all required data read in'
          Errormessage(2) = &
               'satellites with no R-matrix will have reduced processing'
 ...
 ELSE
    IF ( GeneralMode >= DebugMode ) THEN
       Message(1) = 'R-matrices initialised ok'
       CALL Gen_MessageReport( RoutineName,Message )
    END IF
 END IF
