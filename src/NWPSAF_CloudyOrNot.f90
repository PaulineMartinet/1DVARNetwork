...
! upgrade this with separate values for each.
   ...
   IF (Window_Channel > 0) THEN
      BTdifference = BTObserved(UsedChans % Channels(Window_Channel)) - &
           RT_Params % TotalBTs(Window_Channel)
   ELSE
      BTdifference = -1.e10  ! Set to large -ve value so as not to
                             ! result in an automatic clear case.
   END IF

   IF ( BTdifference < CloudAbsThresh_IRWindow .AND. Window_Channel > 0) THEN
      Cloudy = .TRUE.
      IF ( BTdifference < HighCloudAbsThresh_IRWindow ) &
           HighCloud = .TRUE.
   ELSE
      Setflag: IF ( RT_Params % RTSurfaceType /= RTsea ) THEN
         IF ( Cost >= CostThresh_land .AND. &
              BTdifference < CostThresh_IRWindow_land ) THEN
            Cloudy = .TRUE.
         ELSE
            Cloudy = .FALSE.
         END IF
      ELSE
         IF ( Cost >= CostThresh_sea .AND. &
              BTdifference < CostThresh_IRWindow_sea ) THEN
            Cloudy = .TRUE.
         ELSE
            Cloudy = .FALSE.
         END IF
...
IF ( GeneralMode >= VerboseMode ) THEN
   ...
   write(*,*) 'BTdifference:',BTdifference
   IF (Cloudy) THEN
