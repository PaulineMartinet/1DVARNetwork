...
SELECT CASE (Humidity_Units)
 ...
 !    RT_Params % RTguess(Prof_q2) = &
  ...
  CASE (Humidity_PPMV)
     ...
     DO level = 1, Num_RTlevels
     !   IASIlevel_rh_out(level) = &
      !       EXP(RT_Params % RTguess(Prof_FirstQ+level-1)) * H2O_MassMixToPPMV
        IASIlevel_rh_out(level) = &
             RT_Params % RTguess(Prof_FirstQ+level-1) * H2O_MassMixToPPMV
     END DO
 !    RT_Params % RTguess(Prof_q2) = &
  !        EXP(RT_Params % RTguess(Prof_q2)) * H2O_MassMixToPPMV
     RT_Params % RTguess(Prof_q2) = &
          RT_Params % RTguess(Prof_q2) * H2O_MassMixToPPMV
...
Obs% LWP% value = RT_Params % RTguess(Prof_CLW)
Obs% IWV% value = RT_Params % RTguess(Prof_IWV) !PM change
  ...
  CASE(Humidity_PPMV)
     ...
     DO Element=Num_RTLevels,1,-1
        WRITE(FileUnit_Retrieved_Profiles,FMT='(F12.3,2(F10.3,E12.4,E12.4))') &
             ...
             RT_Params % RTBack(Prof_FirstQ+Element-1)*H2O_MassMixToPPMV,&
	!     EXP(RT_Params % RTBack(Prof_FirstQ+Element-1))*H2O_MassMixToPPMV,&
...
CASE(Humidity_PPMV)
 !  WRITE(FileUnit_Retrieved_Profiles, &
  !      FMT='(''Surface Humidity (ppmv):      '', 2F10.3)') &
   !     Obs% rh2% Value,                                    &
       ! EXP(RT_Params % RTBack(Prof_q2))* H2O_MassMixToPPMV
 !       RT_Params % RTBack(Prof_q2)* H2O_MassMixToPPMV
CASE(Humidity_MassMix)
  ! WRITE(FileUnit_Retrieved_Profiles, &
...
IF (MwClwRetrieval .AND. Lqtotal==0) THEN
   ...
   WRITE(FileUnit_Retrieved_Profiles, &
        FMT='(''LWP    (kg/m2):              '', 2F10.7)') &
        Obs% LWP % Value, &
        RT_Params % RT1stguess(Prof_CLW)
...
IF (IWVRetrieval) THEN
   ! N.B. 1st guess CTP and Cloud fraction reported, not background.
  ! WRITE(FileUnit_Retrieved_Profiles, &
  !      FMT='(''IWV    (kg/m2):              '', 2F10.3)') &
  !      Obs% IWV % Value, &
  !      RT_Params % RT1stguess(Prof_IWV)
   !WRITE(FileUnit_Retrieved_Profiles,FMT=*) 
        !OPEN(UNIT=3000,FILE=&
        !&'/home/martinet/1DVAR/NWP_SAF/nwpsaf_1dvar_1_0/&
        !&1DVar_v1.0_brute_force_jacobians_no_lnq_IWV_retrieve/IWV.dat',FORM='FORMATTED',ACCESS='SEQUENTIAL')

        OPEN(UNIT=3000,FILE=&
        &'/home/francesco/Documenti/1DVAR/1DVar_K_complete_clean/IWV.dat',FORM='FORMATTED',ACCESS='SEQUENTIAL')
        write(3000,*)  Obs% IWV % Value, &
        RT_Params % RT1stguess(Prof_IWV)
END IF
...
DO IChan=1,UsedChans % NumChans
   WRITE(FileUnit_Retrieved_BTs,FMT='(I7,3F11.3)') &
        ...
        BTObserved(UsedChans % Channels(IChan)), &
