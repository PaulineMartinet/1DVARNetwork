...
INCLUDE 'Gen_FreeUnit.interface'
...
INTEGER :: &
  fileunit_Bmatrix    = 0, & ! unit number for B-matrix file
  fileunit_Rmatrix          = 0, & ! unit number for R-matrix file
  fileunit_ChannelChoice    = 0
...
CHARACTER(LEN=80) :: Message(numfiles+1)  ! Message for Gen_MessageReport
...
!----
!This is a completely new channel selection method and is still in its
!infancy. The code will not cause any problems during normal use, but
!may lack the complete flexibility desired.
...
IF ( ErrorCode /= 0 ) THEN
   WRITE( UNIT=ErrorMessage(1),FMT='(A,I3)' ) 'Error code = ', ErrorCode
...
!2.8) Set up B-matrix
!----
!Could move allocations outside of NWPSAF_InitBmatrix into this routine
!for consistency

! The  following is now set up in the FileInfo structure and passed directly
! into NWPSAF_InitBmatrix through the NWPSAFMod_Params module.  ADC 11/11/99.:
...
close_files : DO file = numfiles, 1, -1
   ...
   IF (unit_number > 0) THEN
      ...
      END IF
      CALL Gen_FreeUnit(unit_number,ErrStatRep)
   END IF
END DO close_files
IF ( GeneralMode >= DebugMode ) THEN
   CALL Gen_MessageReport( RoutineName,Message(1:numfiles) )
END IF
