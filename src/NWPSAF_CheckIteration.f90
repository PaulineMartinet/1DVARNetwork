...
DO level = 1,Num_RTlevels
   IF ( (Guess_Prof(Prof_FirstT+level-1) > MaxTemperature) .OR. &
      ...
      IF ( GeneralMode >= VerboseMode ) THEN
         WRITE(*,*) 'INVALID DATA: Temperature outside limits at level ',level
      ELSE
         EXIT
      END IF
   END IF
...
Constrain: IF ( Out_of_range ) THEN
...
!!!fradea
  ! Add a new constrain to check that the dela_x is not too far away
  ! from the specification of the Bmatrix
comptage:   DO element1 = 1,Num_ProfElementsUsed
                IF(Profile_incs(element1)>=3*sqrt(Bmatrix(element1,element1))) THEN
                        Profile_Variables_Reset = .TRUE.
                        write(*,*) 'check delta_x plus petit que B',element1, Profile_incs(element1), &
& 3*sqrt(Bmatrix(element1,element1))
                exit comptage
                ENDIF
            ENDDO comptage
!!!fradea   
