...
! Ticket  Date     Comment
! ------- -------- -------
! 31      18/01/13 Original version, based on RTTOV11_interface without
!                  option to read in emissivity atlas     P. Weston.
...
REAL            :: q_profiles(Num_RTlevels) !PM change
...
! Check that there aren't too many instruments
   ...
   RTTOV11_Opts(:) % interpolation % addinterp  = .true.
   ...
   DO I=1,RT_Params%Num_Instruments
     ...
     IF (Nchans(I) > 0) THEN
        CALL RTTOV_READ_COEFS( &
             ...
             instrument = RTInstrument(1:3,I))   ! in
     ELSE
        CALL RTTOV_READ_COEFS( &
             ErrorStatus(I),                           & ! out
             RTTOV11_Coefs(I),                         & ! out
  ...
  !   Soft_Limits % Minimum(Prof_FirstQ:Prof_LastQ) = &
   !       LOG(RTTOV11_Coefs(1) % coef % lim_prfl_gmin(:,wv_pos)/ &
    !      H2O_MassMixToPPMV)-0.5
     Soft_Limits % Minimum(Prof_FirstQ:Prof_LastQ) = &
          RTTOV11_Coefs(1) % coef % lim_prfl_gmin(:,wv_pos)/ &
          H2O_MassMixToPPMV
 ...
 !    Soft_Limits % Minimum(Prof_Q2) = &
  !        LOG(RTTOV11_Coefs(1) % coef % lim_prfl_gmin(Num_RTLevels,wv_pos)/ &
   !       H2O_MassMixToPPMV)-0.5
   Soft_Limits % Minimum(Prof_Q2) = &
          RTTOV11_Coefs(1) % coef % lim_prfl_gmin(Num_RTLevels,wv_pos)/ &
          H2O_MassMixToPPMV
 ...
 !    Soft_Limits % Maximum(Prof_FirstQ:Prof_LastQ) = &
  !        LOG(RTTOV11_Coefs(1) % coef % lim_prfl_gmax(:,wv_pos)/ &
   !       H2O_MassMixToPPMV)+0.5
     Soft_Limits % Maximum(Prof_FirstQ:Prof_LastQ) = &
          RTTOV11_Coefs(1) % coef % lim_prfl_gmax(:,wv_pos)/ &
          H2O_MassMixToPPMV
...
!     Soft_Limits % Maximum(Prof_Q2) = &
 !         LOG(RTTOV11_Coefs(1) % coef % lim_prfl_gmax(Num_RTLevels,wv_pos)/ &
  !        H2O_MassMixToPPMV)+0.5
     Soft_Limits % Maximum(Prof_Q2) = &
          RTTOV11_Coefs(1) % coef % lim_prfl_gmax(Num_RTLevels,wv_pos)/ &
          H2O_MassMixToPPMV
...
ELSE
   ...
   nullify(Profiles(1) % co2, Profiles(1) % n2o, Profiles(1) % co, Profiles(1) % ch4, ProfileS(1)%cloud, Profiles(1)%cfrac, &
& Profiles(1)%icede, profiles(1)%aerosols)
   ...
   DO Variable_Number = 1, Num_Atm_Prof_Vars
     ...
     !    profiles(1)%q(:) = EXP(RTProf(Prof_FirstQ:Prof_LastQ)) * &
      !        H2O_MassMixToPPMV
      profiles(1)%q(:) = RTProf(Prof_FirstQ:Prof_LastQ) * &
              H2O_MassMixToPPMV
...
! Now do the Surface Variables
   DO Variable_Number = 1, Num_Surf_Vars
    ...
    !     profiles(1)%s2m%q = EXP(RTProf(Prof_Q2)) * H2O_MassMixToPPMV
         profiles(1)%s2m%q = RTProf(Prof_Q2)* H2O_MassMixToPPMV
...
! ****** FORWARD MODEL CALL ******
     CASE( FastmodelMode_Forward )
        ...
        Instrument_Loop : DO Instrument = First_Instr, Last_Instr
           ...
           RTTOV11_Chanprof(1:NumInstChans)%prof = KPROF(First_Chan_Pos:Last_Chan_Pos)*1_JPIM
...
!	WRITE(*,*) 'canaux BRUTE force', NumInstChans, First_Chan_Pos, Last_Chan_Pos, Valid_Channels(1:Nchans(2),2)
!STOP
!ENDIF

!	CALL NWPSAF_RTTOV11_Brute_Force_Jacobians ( &
!	NumInstChans ,  & ! total number of channels for the current instrument
!	First_Chan_Pos,  & ! in premier canal correspond a linstrument a etudier
!	Last_Chan_Pos , & ! dernier canal correspondant à linstrument de la boucle en question 
...
!                  radiance%reflclear(1:NumInstChans)
           ...
           ! (LWP) or qtotal
          !--------------------------------------------------------------------
           ! 2.5  Calculate total water vapor path to create the minimalisation variable IWV_calc
           !      in the state vector. NWPSAF_Layers_to_LWP integrates up the
           !      water on the fixed pressure levels.
           !--------------------------------------------------------------------
           IF(IWVRetrieval) THEN ! IWV retrieval
           
              q_profiles =  profiles(1)%q(:)
              CALL NWPSAF_Layers_to_LWP( q_profiles,      & ! In
                                     RT_params,         &
                                     IWV_calc           ) ! Out
	  q_structure(:) = &
          RT_Params % RTBack(Prof_FirstQ:Prof_LastQ)/RT_Params % RTBack(Prof_IWV)
              ...
              !-----------------------------------------------------------------
              ! 2.6 Compute dTb/dIWV = dIWV                                    
              !-----------------------------------------------------------------
              DO i=First_Chan_Pos,Last_Chan_Pos
                 CALL NWPSAF_LayerK_to_LWPK (profiles_k(i)%q(:),         &
                                           q_structure,              &
                                           RTModel_Jacobian(Prof_IWV, i) )
              END DO
           END IF
...
!!POPO 
              DO i=First_Chan_Pos,Last_Chan_Pos
                 CALL NWPSAF_LayerK_to_LWPK (profiles_k(i-First_Chan_Pos+1)%clw(:),         &
                                           cloud_structure,              &
    ...
    !          wqtotal(1:Num_WetLevels)= EXP(RT_Params % RTguess(Prof_LastQ - &
     !              Num_WetLevels + 1:Prof_LastQ)) + RT_Params % RTguess(&
      !             Prof_LastCLW- Num_WetLevels + 1:Prof_LastCLW)
         wqtotal(1:Num_WetLevels)= RT_Params % RTguess(Prof_LastQ - &
                   Num_WetLevels + 1:Prof_LastQ) + RT_Params % RTguess(&
                   Prof_LastCLW- Num_WetLevels + 1:Prof_LastCLW)
