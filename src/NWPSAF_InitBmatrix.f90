...
INTEGER :: Status         ! Status from matrix inversion routine
...
!!!fradea
 !   DO i = 1, Num_Elements 
!	write(*,*)i, Bfromfile(i,i)
 !       Bfromfile(i,i) =Bfromfile(i,i)*2
 !  ENDDO
!!!fradea
   ...
   DO i = 1, Num_ProfElementsUsed
      ...
      ELSE IF (B_ElementsUsed(i) == 0. .AND. &
               Retrieved_Elements(i) == Prof_CLW) THEN
         B_Reference(:,i,type) = 0.0
         B_Reference(i,:,type) = 0.0
         B_Reference(i,i,type) = LwpSD * LwpSD
!=====AJOUT PM MODIFICATION===============================
      ELSE IF (B_ElementsUsed(i) == 0. .AND. &
               Retrieved_Elements(i) == Prof_IWV) THEN
         B_Reference(:,i,type) = 0.0
         B_Reference(i,:,type) = 0.0
         B_Reference(i,i,type) = IWVSD * IWVSD
!==================================================
      ELSE IF (B_ElementsUsed(i) == 0. .AND. &
      ...
      ELSE
         WHERE(B_ElementsUsed(:) > 0) B_Reference(i,:,type) = &
               Bfromfile(B_ElementsUsed(i),B_ElementsUsed(:))
      END IF
   END DO
...
IF ( ErrStatRep == StatusOK ) THEN
   ...
   DO type = 1, BmatrixTypes
      ...
      ! invert
      CALL NWPSAF_SatMatInv( Num_ProfElementsUsed, & ! in
      ...
      IF ( Status /= 0 ) THEN
         ErrStatRep = StatusWarning
         Errormessage(1) = 'Error inverting B matrix'
      END IF
