...
CHARACTER(LEN=80) :: ErrorMessage(5)  ! Message for Gen_ErrorReport
...
Class_loop :  DO j = 1, MaxClasses
    ...
    DO ichannel = 1, Num_Channels
       Use_Channel(ichannel) = &
            BTEST(Channel_Mask(ichannel),i-1) .AND. &
            BTEST(Channel_Mask(ichannel),j+4)
       IF (Use_Channel(ichannel)) Num_Channels_to_Use = & 
            Num_Channels_to_Use + 1
...
DO ichannel = 1, Num_Channels
   ...
   IF (ABS(BG_and_Cloud_Mask(ichannel)) == 2 .OR. &
      ...
      DetectCloudChans_Tmp(DetectCloudChans % numchans) = &
           Channel_Number(ichannel)
