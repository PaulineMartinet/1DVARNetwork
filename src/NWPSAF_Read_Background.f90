...
ALLOCATE( Background % IWV ( NumObs )) ! PM integrated water vapour variable
...
ReadData : DO I = 1, NumProf
  ...
  ! reverse if need be and convert pressure levels from hPa to Pa
  IF (Background % P(I,1) > Background % P(I,2)) THEN
     temparray(1:NumLevels)=100*Background % p(I,1:NumLevels)
     ...
     ENDIF
  ELSE
	Background % p(I,:)=100* Background % p(I,:)
  END IF
  ...
  Background % PStar(I) = Vars(1)*100.0
  IF (ReadStatus /= 0) THEN
    WRITE (ProfNumChar, FMT='(I20)') I 
...
! Cloud liquid water
Prof_CLW        = 4*NumLevels + 1
!INTEGRATED WATER VAPOR
Prof_IWV=4*NumLevels + 2 !PM change
! 1.5m temperature and humidity 
Prof_T2         = 4*NumLevels + 3
Prof_q2         = 4*NumLevels + 4
! Surface pressure
Prof_pstar      = 4*NumLevels + 5
! 10m wind
Prof_uwind      = 4*NumLevels + 6
Prof_vwind      = 4*NumLevels + 7
! Skin temperature
Prof_Tstar      = 4*NumLevels + 8
! Cloud top pressure and effective cloud fraction
Prof_CTP        = 4*NumLevels + 9
Prof_CloudCover = 4*NumLevels + 10
! Total size of vector
ProfSize = 4*NumLevels + 10 !PM change
