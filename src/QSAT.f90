!LL  SUBROUTINE QSAT--------------------------------------------------- 
!LL
!LL  This software was developed within the context of 
!LL  the EUMETSAT Satellite Application Facility on 
!LL  Numerical Weather Prediction (NWP SAF), under the 
!LL  Cooperation Agreement dated 25 November 1998, between 
!LL  EUMETSAT and the Met Office, UK, by one or more partners 
!LL  within the NWP SAF. The partners in the NWP SAF are 
!LL  the Met Office, ECMWF, KNMI and MeteoFrance.
!LL
!LL  Copyright 2004, EUMETSAT, All Rights Reserved.
!LL                                                            
!LL  PURPOSE : RETURNS A SATURATION MIXING RATIO GIVEN                  
!LL            A TEMPERATURE AND PRESSURE USING SATURATION              
!LL            VAPOUR PRESSURES CALCULATED USING THE                    
!LL            GOFF-GRATCH FORMULAE, ADOPTED BY THE WMO AS              
!LL            TAKEN FROM LANDOLT-BORNSTEIN, 1987 NUMERICAL             
!LL            DATA AND FUNCTIONAL RELATIONSHIPS IN SCIENCE             
!LL            AND TECHNOLOGY. GROUP V/VOL 4B METEOROLOGY.              
!LL            PHYSICAL AND CHEMICAL PROPERTIES OF AIR, P35             
!LL                                                                     
!LL            VALUES IN THE LOOKUP TABLE ARE OVER WATER ABOVE          
!LL            0 DEG C AND OVER ICE BELOW THIS TEMPERATURE              
!LL                                                                     
...
!                                                                       
      REAL, PARAMETER    :: T_LOW    = 183.15
                            ! LOWEST TEMPERATURE FOR WHICH LOOK-UP       
                            ! TABLE OF SATURATION WATER VAPOUR           
                            ! PRESSURE IS VALID (K)                      
!                                                                       
      REAL, PARAMETER    :: T_HIGH  = 338.15
                           ! HIGHEST TEMPERATURE FOR WHICH LOOK-UP      
                           ! TABLE OF SATURATION WATER VAPOUR           
...
!                                                                       
      INTEGER , PARAMETER:: N  = 1551 !  =INT(((T_HIGH - T_LOW + 
                           !      (DELTA_T*0.5))/DELTA_T) + 1.0)        
                           ! SIZE OF LOOK-UP TABLE OF SATURATION        
...
!---------------------------------------------------------------------- 
!                                                                       
      REAL, INTENT(IN) :: T(NPNTS)  ! IN TEMPERATURE (K)  
!                                                                       
      REAL, INTENT(IN) :: P(NPNTS)  ! IN PRESSURE (Pa)
!                                                                       
!                                                                       
!---------------------------------------------------------------------- 
...
!---------------------------------------------------------------------- 
!                                                                       
      REAL, INTENT(OUT) :: QS(NPNTS) ! OUT SATURATION MIXING RATIO AT TEMPERATURE 
...
!                                                                       
      REAL, PARAMETER :: ONE_MINUS_EPSILON  = 1.0 - EPSILON
                                 ! ONE MINUS THE RATIO OF THE MOLECULAR    
...
!                                                                       
      DO I=1,NPNTS                                                   
!      COMPUTE THE FACTOR THAT CONVERTS FROM SAT VAPOUR PRESSURE        
!      IN A PURE WATER SYSTEM TO SAT VAPOUR PRESSURE IN AIR, FSUBW.     
!      THIS FORMULA IS TAKEN FROM EQUATION A4.7 OF ADRIAN GILL'S        
!      BOOK: ATMOSPHERE-OCEAN DYNAMICS. NOTE THAT HIS FORMULA           
!      WORKS IN TERMS OF PRESSURE IN MB AND TEMPERATURE IN CELSIUS,     
!      SO CONVERSION OF UNITS LEADS TO THE SLIGHTLY DIFFERENT           
!      EQUATION USED HERE.                                              
!                                                                       
       FSUBW = 1.0 +1.0E-8*P(I)*( 4.5 + &
         6.0E-4*( T(I) - ZERODEGC )*( T(I) - ZERODEGC ) )               
!                                                                       
!      USE THE LOOKUP TABLE TO FIND SATURATED VAAPOUR PRESSURE,         
!      AND STORE IT IN QS.                                              
!                                                                       
!                                                                       
       TT=MAX(T_LOW,T(I))                                               
       ...
       ATABLE = (TT - T_LOW + DELTA_T) / DELTA_T                        
          ITABLE = INT(ATABLE)                                               
          ATABLE = ATABLE - REAL(ITABLE)                                      
!                                                                       
          QS(I) = (1.0 - ATABLE)*ES(ITABLE) &
                 + ATABLE*ES(ITABLE+1)                                  
!                                                                       
!                                                                       
!     MULTIPLY BY FSUBW TO CONVERT TO SATURATED VAPOUR PRESSURE         
!     IN AIR (EQUATION A4.6 OF ADRIAN GILL'S BOOK).                     
!                                                                       
       QS(I) = QS(I)*FSUBW                                              
!                                                                       
!     NOW FORM THE ACCURATE EXPRESSION FOR QS, WHICH IS A               
!     REARRANGED VERSION OF EQUATION A4.3 OF GILL'S BOOK.               
!                                                                       
!     NOTE THAT AT VERY LOW PRESSURES WE APPLY A FIX, TO                
