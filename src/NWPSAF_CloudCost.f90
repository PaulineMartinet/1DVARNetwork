...
INTEGER :: I
REAL :: BTdifference (UsedChans % NumChans)
...
CASE (R_Band_Diagonal)
   DO Element = 0,R_SubMatrix % Num_Elements
      DO I=1, UsedChans % NumChans - Element
         U(I,I+Element) = U(I,I+Element) + &
              R_SubMatrix % Matrix(Element,I)
...
BTdifference(:) = &
     BTObserved(UsedChans % Channels(1 : UsedChans % NumChans)) - &
     RT_Params % TotalBTs(1 : UsedChans % NumChans)

!-----------------------------------------------------------------------
! 5. Calculate U^-1.BTdifference using Cholesky decomposition and 
!    then premultiply by BTdifference^T to get the cloud cost.
!-----------------------------------------------------------------------
...
IF (ErrorCode /= 0) THEN
   Errormessage(1) = 'Cholesky decomposition error '
   ErrStatRep = StatusFatal
   CALL Gen_ErrorReport( RoutineName,  & ! in
        ErrorMessage, & ! in
        ErrStatRep )    ! in
...
Cost = 0.5*DOT_PRODUCT(BTdifference,Scratch_matrix)
