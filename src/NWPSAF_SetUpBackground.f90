...
Humidities: IF ( Valid_Data ) THEN
   ...
   CALL QSAT( qsaturated(Num_RTLevels+1), &
        SurfaceT,                         &
        SurfaceP,                         &
        1 )
 ...
 !            LOG(rhumidity / H2O_MassMixtoPPMV)
 ...
 !  END SELECT
   ...
   IF (Humidity_Units == Humidity_PPMV) &
        qsaturated(1:Num_RTLevels) = &
        qsaturated(1:Num_RTLevels) * H2O_MassMixtoPPMV
...
CALL NWPSAF_Layers_to_LWP(RT_Params % RTBack(Prof_FirstCLW:Prof_LastCLW),RT_Params, LWP_back)
RT_Params % RTBack(Prof_CLW)=LWP_back

!---------------------
!5. Integrated Water vapor
!---------------------
CALL NWPSAF_Layers_to_LWP(RT_Params % RTBack(Prof_FirstQ:Prof_LastQ),RT_Params, IWV_back)
