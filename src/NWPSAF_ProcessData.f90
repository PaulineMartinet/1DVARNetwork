...
USE NWPSAFMod_Params, ONLY : &
  ...
  !Various constants
    QC_BackgroundProf, &
    FileUnit_Retrieved_Profiles, FileUnit_Retrieved_BTs, &
...
INTEGER :: RTerrorcode
REAL    :: BTBackground(Total_channels)       !Background BTs
REAL    :: BTObserved(Total_channels)         !Observed (bias corrected) BTs
REAL    :: BTRetrieved(Total_channels)        !Retrieved BTs
...
ALLOCATE(RT_Params % RTGuess_BF(ProfSize))
...
Loop: DO obnumber = FirstOb, LastOb
   ...
   !Initialize variables1DVAR/working/NWPSAF_1DVar
   BTBackground(:) = MissData_R
   BTRetrieved(:)  = MissData_R
   ...
   IF ( ErrorCode /= 0 ) THEN
      write(*,*) 'Bad background data, ErrorCode =',ErrorCode
      Valid_Data = .FALSE.
      Background% BriTemp(:) = MissData_R
      CYCLE Loop
   END IF
   ...
   !2.6.4) Transfer calculated values into 1DVar variables
   !------
   ...
   DO i = 1, UsedChans % NumChans
      ...
      BTBackground(channel) = RT_Params % TotalBTs(i)
      BTdifference = BTBackground(channel) - BTObserved(channel)
      IF (CloudyRetrieval) THEN
   ...
   Background% BriTemp(:) = BTBackground(:)
   ...
   SetMatrices: IF ( DoCloudDetection .OR. DoOneDVar .OR. CloudyRetrieval) THEN
   ...
   DetectCloudBlock: IF ( DoCloudDetection ) THEN
      ...
      IF ( RTerrorcode > 0 ) THEN
         IF ( GeneralMode >= DebugMode ) THEN
            ...
            Output_format = Reject_format // ",'" // Output_text // "')"
            !xxx            WRITE(*,Output_format) Obs% Id(obnumber), &
            !xxx            Obs % latitude % value, &
            !xxx            Obs % longitude % value, &
            !xxx            RTsurftype_text(RT_Params % RTsurfacetype)
         END IF
      ...
      IF ( .NOT. GoodBackChannels(UsedChans % Channels(channel)) .OR. &
         ...
         IF ( GeneralMode >= DebugMode ) THEN
            WRITE( UNIT=Output_text,FMT='(A,I5,A)' ) &
                 ' channel',UsedChans % Channels(channel), &
                 ' failed o-b check'
            WRITE(*,*) Output_Text
         END IF
         EXIT Check_channels
   ...
   Perform1DVarBlock: IF ( DoOneDVar .AND. Valid_data ) THEN
      ...
      BTRetrieved(UsedChans % Channels(1 : UsedChans % NumChans)) = &
           RT_Params % TotalBTs(1 : UsedChans % NumChans)
      Obs% BriTemp(:) = BTRetrieved(:)
      ...
      ! --------------------------------------
      IF ( Valid_data .and. converged) THEN
   ...
   IF ( GeneralMode >= VerboseMode ) THEN
      ...
      DO i = 1, UsedChans % NumChans
         ...
         WRITE(*,'(I7,5F9.2)') channel, &
              BTObserved(channel),BTBackground(channel), &
              BTRetrieved(channel), &
              BTObserved(channel)-BTRetrieved(channel), &
              BTObserved(channel)-BTBackground(channel)
      END DO
   END IF
...
DEALLOCATE (RT_Params % RTGuess_BF)
DEALLOCATE (RT_Params % RT1stGuess)
DEALLOCATE (RT_Params % TotalBTs)
DEALLOCATE (RT_Params % TotalRadiances)
IF (CloudyRetrieval) THEN
