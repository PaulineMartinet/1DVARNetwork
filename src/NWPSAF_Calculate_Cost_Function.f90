  ...
  CASE (R_Full_Matrix)
     IF (Eqn_101 .AND. .NOT. R_SubMatrix % Inverse) THEN
        ...
        DO I = 1, R_SubMatrix % Num_Chans
           Jo = Jo + DeltaBT(I) * DeltaBT(I) / R_SubMatrix % Matrix(I,I)
        END DO
     ELSE    
        ! This is the full calculation 
        Scratch_VectorO(:) = DeltaBT(:)
        IF (.NOT. R_SubMatrix % Inverse) THEN
           ...
           DEALLOCATE(AI)
        ELSE 
           Scratch_VectorO(:) = MATMUL(R_SubMatrix % Matrix(:,:), DeltaBT(:) )
        END IF
        Jo = DOT_PRODUCT(DeltaBT(:), Scratch_VectorO(:))
     END IF
!-----------------------------------------------------------------------------
! 2.2. Band diagonal matrix representation.
!-----------------------------------------------------------------------------
  CASE (R_Band_Diagonal)
     !  Diagonal Case (or "poor man's cost function")
     IF (R_SubMatrix % Num_Elements == 0 .OR. Eqn_101) THEN
        IF (R_SubMatrix % Inverse) THEN
           Scratch_VectorO(:) = DeltaBT(:) * R_SubMatrix % Matrix(0,:)
        ELSE
           Scratch_VectorO(:) = DeltaBT(:) / R_SubMatrix % Matrix(0,:)
        END IF
        Jo = DOT_PRODUCT(DeltaBT(:),Scratch_VectorO(:))
     ELSE
        ...
        IF (.NOT.(R_SubMatrix % Inverse)) THEN
           ...
           ALLOCATE(R_SubMatrix % Matrix(0:Max_Elements, &
                R_SubMatrix % Num_Chans))
     ...
     IF (R_SubMatrix % Inverse) THEN
        ...
        IF (PRESENT(JCost_Gradient)) THEN
           IF (PRESENT(H_Matrix_T)) THEN
              JCost_Gradient(:) = MATMUL( MATMUL( &
                   H_Matrix_T(:,:),TRANSPOSE(R_SubMatrix % Matrix(:,:))), &
                   Scratch_VectorO(:) * R_SubMatrix % EigenValues(:))
           ELSE
              Errormessage(1) = &
