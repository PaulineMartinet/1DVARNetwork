...
!     Description:
!     Calculate radiances for IASI selected channels.
!
!     IASRTM--->RTIAI--->IASCF             ------------------------------
!           |                                                            
!           |                                                            
...
!     1.1          15/10/1999   Modified to be direct model on 1DVar. 
!                                                     Andrew Collard UKMO
!
!     Code description:
!       Language:              Fortran 90.
...
!     Module used:
  ...
  INTEGER, INTENT(IN) :: KLENPF  ! Number of profile elements
  ...
  REAL, INTENT(OUT)    :: PRAD_D(KNCHPRO * KNPF)  ! Input profile
  REAL, INTENT(OUT)   :: TB_D(KNCHPRO * KNPF)  ! Output brightness temperature
  ...
  INTEGER, PARAMETER :: KNCHTRS=1    ! Number of channels for which
  ...
  INTEGER            :: J,I,K
  INTEGER            :: ID                    ! Input satellite index.
  ...
  REAL :: TAUM_D(KNCHPRO * KNPF,JPLEV)
  REAL :: TAUW_D(KNCHPRO * KNPF,JPLEV)
  REAL :: TAUO_D(KNCHPRO * KNPF,JPLEV)
  REAL :: TAUSFC_D(KNCHPRO * KNPF) 
  REAL :: TAU_D(KNCHPRO * KNPF,JPLEV) 
  REAL :: EMS_D(KNCHPRO * KNPF)  


  KNCHPF = KNCHPRO * KNPF  

  !----- For now we only deal with one profile at a time.  So check that--
  !----- KNPF is one and exit if not.-------------------------------------
  ...
  !-----------------------------------------------------------------------
  !          4.    PERFORM DIRECT RADIATIVE TRANSFER CALCULATIONS
  !-----------------------------------------------------------------------




!/////PROF_D(KLENPF,KNPF): Input vector to RTIASI.
...
!///////////////////////////////////////////////////////////////////////
  ...
  CALL RTIASI(PROF_D,KNPF,KLENPF,PANGL,PANGS,KSURF,KCHAN,KPROF, &
       KNCHPF,PRAD_D,TB_D,ID,TAUM_D,TAUW_D,TAUO_D, &
       TAU_D,TAUSFC_D,EMS_D,IPRINT)

END SUBROUTINE RTIASI_Direct
