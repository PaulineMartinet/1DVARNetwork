...
CHARACTER(LEN=50) :: ErrorMessage(2)  ! Message for Gen_ErrorReport
...
!!!!MODIFICATION PAULINE
 ALLOCATE(RT_Params % view_angle(RT_Params % Num_Instruments))
   ...
   ALLOCATE(RT_Params % &
        First_Channel_for_Instrument(RT_Params % Num_Instruments))
   ALLOCATE(RT_Params % &
        Instrument_Number(RT_Params % Num_SatIDs, NumChannels))
   ...
   DO I = 1, RT_Params % Num_Instruments
      READ (UNIT=fileunit, FMT=* , IOSTAT=ReadStatus) &
           ...
           Last_Channel,                  &
           WMO ,                             &
	   RT_Params % view_angle(I) !!!MODIFICATION PAULINE 
      ...
      RT_Params % Instrument_Number( &
           IComp,First_Channel:Last_Channel) = I
   ...
   DO Instr = 1, RT_Params % Num_SatIDs
      IF (.NOT.(ALL(RT_Params % Instrument_Number(Instr,:) == 0))) THEN
         IF (ANY((RT_Params % Instrument_Number(Instr,:) <= 0) .OR. &
            ...
            Errormessage(1) = &
                 'Not all channels have been assigned an instrument '
...
!write(*,*) 'FIn de la lecture de lobservation   ', Observations % BriTemp(:)
! 
